...
WINDOWS 10:
- open regedit
- navigate to HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\W32Time and
...
Note: When you change to Automatic (Delayed Start) a new key DelayedAutostart is
...
/v Enabled /t REG_DWORD /d 0x1 /f w32tm /config /syncfromflags:manual
/reliable:yes /update
...
then disable windows firewall and retry
*/
...
// timestamp is valid if it's greater than 2018-01-01
#define TIMESTAMP_VALIDATION 1514764801
...
class Timeline {
    // this is an offset that has to be added to the server's
    // clock to generate a valid timestamp for the packet
    clock_t start_offset;

    time_t central_clock;
    struct tm timeinfo;

    // concurrent access to timeinfo variable
    mutex m; 
...
public:
    ...
    time_t get_time()
    {
        ...
        return central_clock +
               ((clock() - start_offset) / ((time_t)CLOCKS_PER_SEC));
    }
