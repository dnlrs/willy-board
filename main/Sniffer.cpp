...
Sniffer::start()
{
    // initialize the system timeline global object (global to all sources)
    ...
    xTaskCreatePinnedToCore(Sniffer::sender_task, "sender", STACK_SIZE,
                            (void*)&server_addr, 2, &sender_handle, 1);
    ...
    // main_task (at core 0) now becomes the sniffer_task (specified below)
    Sniffer::sniffer_task(NULL);
}
...
Sniffer::sender_task(void* pvParameters)
{
    ...
    // initialize socket for server connection
    NetWrap net_handler((server_addr->ip).c_str(), server_addr->port);

    // connect to the server
    bool server_connection_alive = net_handler.connect();
    bool wifi_connection_alive   = true;
    // at this point we cannot detect wifi problems.
    // wifi problems can be detected only when trying many times to connect to
    // the server without success or by handling errno "host unreachable"
    ...
    system_timeline->initialize_sntp(
        const_cast<char*>((server_addr->ip).c_str()));

    if (wifi_connection_alive) {
        // this instruction blocks the task on a condition variable
        system_timeline->sync_time();
        system_timeline->print_time();
    }

    int num_attempts = 0;
    clock_t start_refresh_time; // measures how much time is passed since the
                                // last call of sync_time()
    for (;;) {
        ...
        while (server_connection_alive && wifi_connection_alive) {
            ...
            if (p.get_timestamp() > TIMESTAMP_VALIDATION) {
                ...
                // timestamp is not valid when packet was sniffed while
                // connection to the server or to wifi was not alive
                ESP_LOGI(TAG, "wrong timestamp. packet discarded");

            vTaskDelay(500 / portTICK_RATE_MS);

            // check for central clock updates
            if (((clock() - start_refresh_time) / (int)CLOCKS_PER_SEC) >=
                TIME_REFRESH_RATE) {
                system_timeline->sync_time();
                start_refresh_time = clock();
            }
        ...
        if (wifi_connection_alive) {
            ...
            server_connection_alive =
                net_handler.connect(); // attempt to connect to server with the
                                       // new socket
            ...
            if (!server_connection_alive) {
                ...
                if (num_attempts > SERVER_CONNECTION_ATTEMPTS)
                    wifi_connection_alive =
                        false; // too many attempts. maybe it's a wifi problem
            }
            else
                system_timeline->sync_time();
        }
...
Sniffer::incoming_packet_cb(void* buff, wifi_promiscuous_pkt_type_t type)
{
    ...
    if (p.fetchData(buff, NULL) == 0) {
        if (packet_queue->timed_push(p,
                                     5)) // wait for push for at most 5 seconds
            cout << "pushed" << endl;
        else
